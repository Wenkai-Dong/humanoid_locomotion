...
def joint_torque_limits(
    ...
    """Penalize joint torque if they cross the soft limits.

    This is computed as a sum of the absolute value of the difference between the applied torque and the soft limits.
...
def straight_orientation_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Penalize non-flat base orientation for specified bodies.

    This is computed by penalizing the xy-components of the projected gravity vector for all the bodies specified in `asset_cfg.body_ids`.
    """
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    selected_bodies_quat = asset.data.body_quat_w[:, asset_cfg.body_ids, :]
...
def joint_deviation_l2(
    ...
    asset: Articulation = env.scene[asset_cfg.name]
    # compute out of limits constraints
    position_rel = asset.data.joint_pos[:, asset_cfg.joint_ids] - asset.data.default_joint_pos[:, asset_cfg.joint_ids]
    deviation = (torch.square(position_rel) - threshold).clip(min=0.0)
...
def no_fly(env, sensor_cfg: SceneEntityCfg, threshold: float = 1.0, grace_period_steps: int = 5) -> torch.Tensor:
    ...
    grace_mask = (env.episode_length_buf >= grace_period_steps).float()
    return 1.0 * zero_contact * grace_mask
...
def stand_still_velocity(
    ...
    reward = torch.sum(torch.abs(asset.data.joint_vel), dim=1)
    cmd_norm = torch.norm(env.command_manager.get_command(command_name), dim=1)
