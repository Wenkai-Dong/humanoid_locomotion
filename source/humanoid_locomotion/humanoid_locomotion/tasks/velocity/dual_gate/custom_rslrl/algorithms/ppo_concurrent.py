...
class PPOConcurrent:
    ...
    def __init__(
        ...
        self.concurrent_params = list(chain(
            self.actor.cnn1d.parameters(),
        ))
        autoencoder_ids = {id(p) for p in self.concurrent_params}
        self.ppo_params = [
        ...
        self.optimizer = resolve_optimizer(optimizer)([
            ...
            {"params": self.concurrent_params, "lr": 0.001, "name": "estimator"},
        ])  # type: ignore
    ...
    def update(self) -> dict[str, float]:
        ...
        for batch in generator:
            ...
            concurrent_loss = F.mse_loss(self.actor.latent_privilege.float(), privilege[:, 99:])
            loss = loss + velocity_loss + concurrent_loss
            ...
            nn.utils.clip_grad_norm_(self.concurrent_params, 10.0)
            self.optimizer.step()
            ...
            # Concurrent loss
            if concurrent_loss is not None:
                mean_concurrent_loss += concurrent_loss.item()
            # Explained Variance
            with torch.inference_mode():
        ...
        mean_concurrent_loss /= num_updates
        ...
        loss_dict["concurrent"] = mean_concurrent_loss
