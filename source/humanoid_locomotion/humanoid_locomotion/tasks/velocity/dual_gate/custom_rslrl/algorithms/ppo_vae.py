...
class PPOVAE:
    ...
    def __init__(
        ...
        self.vae_params = list(chain(
            ...
            self.actor.right_mean_latent.parameters(),
            self.actor.right_logvar_latent.parameters(),
            self.actor.encode_mean_vel.parameters(),
            self.actor.encode_logvar_vel.parameters(),
    ...
    def update(self) -> dict[str, float]:
        ...
        for batch in generator:
            ...
            recon_loss= F.mse_loss(torch.stack([self.actor.left_privilege.float(), self.actor.right_privilege.float()], dim=1),
                                   privilege[:, 99:].reshape(-1, 2, 30))
            left_kl_loss= -0.5 *  torch.mean(torch.sum(1 + self.actor.left_logvar.float() - self.actor.left_mean.float().pow(2)
                                                       - self.actor.left_logvar.float().exp(), dim=-1))
            right_kl_loss = -0.5 * torch.mean(torch.sum(1 + self.actor.right_logvar.float() - self.actor.right_mean.float().pow(2)
                                                        - self.actor.right_logvar.float().exp(), dim=-1))
            kl_loss = left_kl_loss + right_kl_loss
            loss = loss + velocity_loss + recon_loss + 1.0 * kl_loss
        ...
        loss_dict["recon"] = mean_recon_loss
        loss_dict["kl"] = mean_kl_loss
