...
class GatedMHAModel(MLPModel):
    ...
    def get_latent(
        ...
        # pool
        maxpool_mapping = self.pool(latent_mapping).max(dim=1, keepdim=True)[0]    # (N, 1, 64)
        latent_1d_encoder = self.linear(torch.concat([latent_1d_encoder, maxpool_mapping], dim=-1))   # (N, 1, 64)
        # gated_mha
