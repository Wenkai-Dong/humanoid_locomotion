...
class AEModel(MLPModel):
    ...
    def __init__(
        ...
        if obs_set == "actor":
            ...
            self.decoder_left = MLP(16, 30, (64,), activation)
            self.decoder_left.init_weights([2**0.5, None, 0.001])
            self.decoder_right = MLP(16, 30, (64,), activation)
            self.decoder_right.init_weights([2**0.5, None, 0.001])
    ...
    def get_latent(
        ...
        if self.obs_groups[0] == "actor":
            ...
            velocity, privilege_l, privilege_r = velocity.detach(), privilege_l.detach(), privilege_r.detach()
            privilege_l_linear = self.linear_z(privilege_l)
            privilege_r_linear = self.linear_z(privilege_r)
        latent_1d_linear = self.linear(torch.cat([velocity, latent_1d], dim=-1))    # (N, 64)
        latent_1d_encoder = torch.stack([latent_1d_linear, privilege_l_linear, privilege_r_linear], dim=1)    # (N, 3, 64)
    ...
    def ae(
        ...
        privilege_en_l = self.encoder_left(latent_history)
        privilege_en_r = self.encoder_right(latent_history)

        self.privilege_de_l = self.decoder_left(privilege_en_l)
        self.privilege_de_r = self.decoder_right(privilege_en_r)
