...
class AME2Model(MLPModel):
    ...
    def __init__(
        ...
        if obs_set == "actor":
            self.proprioception_encoder = MLP(obs_dim_1d + 3, 64, (128,), activation)
            self.proprioception_encoder.init_weights(
                [2 ** 0.5, None, 1.0]
            )
        else:
            self.proprioception_encoder = MLP(obs_dim_1d, 64, (128,), activation)
            self.proprioception_encoder.init_weights(
                [2 ** 0.5, None, 1.0]
            )
        ...
        self.pointwise_mlp = nn.Sequential(
            nn.ELU(),
            MLP(64, 96, (128,), activation)
        )
        self.pointwise_mlp[1].init_weights(
            [2 ** 0.5, None, 1.0]
        )
        ...
        self.global_mlp[1].init_weights(
            [2 ** 0.5, None, 1.0]
        )
        self.proprioception_mlp = nn.Sequential(
            nn.ELU(),
            MLP(128, 96, (256,), activation)
        )
        self.proprioception_mlp[1].init_weights(
            [2 ** 0.5, None, 1.0]
        )

        self.q_norm = nn.LayerNorm(96)
        self.k_norm = nn.LayerNorm(96)
        self.mha = nn.MultiheadAttention(embed_dim=96, num_heads=32, batch_first=True)
        self.o_norm = nn.LayerNorm(96)
    ...
    def get_latent(
        ...
        local_embedding = latent_cnn.flatten(2).permute(0, 2, 1) # (N, 234, 48)
        positional_embedding = self.position(obs["actor_map"].flatten(2).permute(0, 2, 1))   # (N, 234, 16)
        pointwise_local_features = self.pointwise_mlp(torch.cat([local_embedding, positional_embedding], dim=-1))   # (N, 234, 96)
        global_features = self.global_mlp(pointwise_local_features).max(dim=1, keepdim=True)[1] # (N, 1, 64)
        query = self.proprioception_mlp(torch.cat([global_features, proprioception_embedding], dim=-1))
        query = self.q_norm(query)
        key = self.k_norm(pointwise_local_features)
        latent_mha, _ = self.mha(query, key, pointwise_local_features, need_weights=False)    # (N, 1, 96)
        latent_mha = self.o_norm(latent_mha).flatten(1) # (N, 96)
        # Concatenate 1D and CNN latents
        return torch.cat([proprioception_embedding.flatten(1), latent_mha, global_features.flatten(1)], dim=-1)   # (N, 64+64+96)
