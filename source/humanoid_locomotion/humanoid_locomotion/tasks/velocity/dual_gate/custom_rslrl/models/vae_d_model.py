...
class VAEModel(MLPModel):
    ...
    def __init__(
        ...
        if obs_set == "actor":
            ...
            self.left_mean_latent = nn.Linear(64, 8)
            self.left_logvar_latent = nn.Sequential(
                nn.Linear(64, 8),
    ...
    def get_latent(
        ...
        if self.obs_groups[0] == "actor":
            velocity, left_code, right_code = self.vae(obs)
            velocity, left_code, right_code = velocity.detach(), left_code.detach(), right_code.detach()
            left_code = self.left_norm(left_code)
            right_code = self.right_norm(right_code)
            latent_1d_l = torch.cat((velocity, latent_1d, left_code), dim=-1)
            latent_1d_r = torch.cat((velocity, latent_1d, right_code), dim=-1)
        ...
        return torch.cat([velocity, latent_1d, latent_mha, left_code, right_code], dim=-1)   # (N, 355)
    ...
    def vae(
        ...
        self.left_mean = self.left_mean_latent(latent_history)
        self.left_logvar = self.left_logvar_latent(latent_history)
        self.right_mean = self.right_mean_latent(latent_history)
        self.right_logvar = self.right_logvar_latent(latent_history)
        mean_vel = self.encode_mean_vel(latent_history)
        logvar_vel = self.encode_logvar_vel(latent_history)

        left_code = self.reparameterise(self.left_mean, self.left_logvar)
        right_code = self.reparameterise(self.right_mean, self.right_logvar)
        self.velocity = self.reparameterise(mean_vel, logvar_vel)

        self.left_privilege = self.left_decoder(torch.cat([left_code, self.velocity], dim=-1))
        self.right_privilege = self.right_decoder(torch.cat([right_code, self.velocity], dim=-1))

        return self.velocity, left_code, right_code
