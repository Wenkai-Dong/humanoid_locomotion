...
class ConcurrentModel(MLPModel):
    ...
    def __init__(
        ...
        if obs_set == "actor":
            self.linear = nn.Linear(obs_dim_1d + 3 + 30, 64)
        else:
    ...
    def get_latent(
        ...
        if self.obs_groups[0] == "actor":
            velocity, privilege_l, privilege_r = self.estimator(obs)
            velocity, privilege_l, privilege_r = velocity.detach(), privilege_l.detach(), privilege_r.detach()
            latent_1d_l = torch.cat((velocity, latent_1d, privilege_l), dim=-1)
            latent_1d_r = torch.cat((velocity, latent_1d, privilege_r), dim=-1)
        ...
        return torch.cat([velocity, latent_1d, latent_mha, privilege_l, privilege_r], dim=-1)   # (N, 355)
    ...
    def estimator(
        ...
        self.velocity = latent_history[:, :3]
        self.latent_privilege = latent_history[:, 3:]
        privilege_l = latent_history[:, 3:33]
        privilege_r = latent_history[:, 33:]
        return self.velocity, privilege_l, privilege_r
