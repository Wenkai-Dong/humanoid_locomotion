...
class MoeLatentModel(MLPModel):
    ...
    def get_latent(
        ...
        latent_1d = super().get_latent(obs) # (B, 99)
        proprioception_embedding = self.ProprioceptionEncoder(latent_1d)    # (B, 64)
        # Process 2D observation groups with CNNs
        latent_cnn_list = [self.cnns[obs_group](obs[obs_group][:,2:3,:,:]) for obs_group in self.obs_groups_2d] # [(B, 32, 4, 5),]
        local_embedding = torch.cat(latent_cnn_list, dim=-1).flatten(start_dim=1)   # (B, 640)
        ...
        return torch.einsum('be,beo->bo', gate_weights, expert_output)  # (B, 128)
