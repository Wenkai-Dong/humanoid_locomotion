...
class VAEModel(MLPModel):
    ...
    def get_latent(
        ...
        if self.obs_groups[0] == "actor":
            ...
            latent_1d_l = torch.cat((velocity.detach(), latent_1d, left_code), dim=-1)
            latent_1d_r = torch.cat((velocity.detach(), latent_1d, right_code), dim=-1)
        ...
        return torch.cat([velocity.detach(), latent_1d, latent_mha, left_code, right_code], dim=-1)   # (N, 355)
