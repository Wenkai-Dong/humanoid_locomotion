...
class AME1Model(MLPModel):
    ...
    def __init__(
        ...
        # Create Linear encoders
        if linear is not None:
            print("Sharing Linear encoders between models.")
        else:
            # Create CNNs for each 2D observation
            linear = nn.Linear(in_features=obs_dim_1d, out_features=linear_cfg["out_features"], )

        # Create MHA encoders
        if mhas is not None:
            print("Sharing MHA encoders between models, the MHA configurations of the receiving model are ignored.")
        else:
            # Create a mha config for each 2D observation group in case only one is provided
            if not all(isinstance(v, dict) for v in mha_cfg.values()):
                mha_cfg = {group: mha_cfg for group in self.obs_groups_2d}
            # Create CNNs for each 2D observation
            mhas = {}
            for idx, obs_group in enumerate(self.obs_groups_2d):
                mhas[obs_group] = MHA(
                    input_dim_q=linear_cfg["out_features"],
                    input_channels_q=1,
                    input_dim_kv=self.cnn_latent_channels+3,
                    **mha_cfg[obs_group],
                )

        # Compute latent dimension of the MHAs
        for mha in mhas.values():
            self.mha_latent_dim = mha.output_dim  # type: ignore
        ...
        if isinstance(mhas, nn.ModuleDict):
            self.mhas = mhas
        else:
            self.mhas = nn.ModuleDict(mhas)
        # Register Linear encoders
        self.linear = linear
    ...
    def get_latent(
        ...
        cnn_obs = obs["policy_map"].flatten(start_dim=2)
        latent_cnn = torch.cat((cnn_obs, latent_cnn),dim=1).permute(0,2,1)
        latent_mha_list = [self.mhas[obs_group](latent_1d_enc, latent_cnn) for obs_group in self.obs_groups_2d]
        latent_mha = torch.cat(latent_mha_list, dim=-1)
        # Concatenate 1D and CNN latents
        return torch.cat([latent_1d, latent_mha], dim=-1).squeeze(1)
    ...
    def _get_latent_dim(self) -> int:
        return self.obs_dim + self.mha_latent_dim
